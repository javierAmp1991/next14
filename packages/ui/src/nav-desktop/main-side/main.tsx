...
export default function Main({
  ...
  useLayoutEffect(() => {
    options.forEach((item: NavOptionsProps) => {
      if (item.UrlKey === link) {
        ...
        const newPosition =
          pRect.top - containerRect.top + refParent.current!.scrollTop;
  ...
  useLayoutEffect(() => {
    if (refParent.current && refCont.current) {
      const isScroll =
        refParent.current.scrollHeight > refCont.current.offsetHeight - 32;
    ...
    <div className={`${style.main} ${!isOpen && style.short}`}>
      <div className={`${style.gridTitle} ${!isOpen && style.gridTitleIpad}`}>
        <button
          ...
          className={`${style.logo} ${!isOpen && style.logoIpad}`}
        >
          <Image alt="" layout={"fill"} src={SPIX_ICON} />
        </button>
        <div className={`${style.title} ${!isOpen && style.titleIpad}`}>
          {title}
        </div>
      </div>
      ...
      <div ref={refCont} className={style.contItems}>
        <div
          ...
          ref={refParent}
          className={style.gridItems}
        >
          {options.map((item: NavOptionsProps) => (
            <NavItem link={link} key={item.Id} isIpad={!isOpen} item={item} />
          ))}
          <div
            style={t}
            className={`${style.selected} ${!isOpen && style.selectedIpad}`}
          />
          <div
            style={t}
            className={`${style.bar} ${!isOpen && style.barIpad}`}
          />
        </div>
      </div>
      ...
      <div className={style.bottom}>
        {scroll.IsVisible && (
          <div
            ...
            className={`${style.contArrow} ${scroll.State && style.flip}`}
          >
        ...
        <Link href={"/"}>
          <span className={style.return}>{returnText}</span>
        </Link>
      </div>
    </div>
  );
