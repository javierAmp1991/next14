...
export const HeaderPage = () => {
    return (
        <div className={style.grid}>
            <Link href={"/"} as={"/"}>
                <div className={style.sizeLogo}><Image alt="" priority={true} layout={"fill"} src={ICON_HEADER}/></div>
            </Link>

            <Section link={CONST.Loc.Link} image={CONST.Loc.Image} fText={CONST.Loc.FirstText} sText={CONST.Loc.SecondText}/>

            <div className={style.gridSearchBar}>
                <input placeholder={"Buscar servicio, producto o evento"} type="text"/>
            ...
            <Section link={CONST.User.Link} image={CONST.User.Image} fText={CONST.User.FirstText} sText={CONST.User.SecondText}/>
        </div>
    )
}

const Section = ({link, image, fText, sText}:{ link: string, image: string, fText: string, sText: string }) =>
    <Link href={link}>
