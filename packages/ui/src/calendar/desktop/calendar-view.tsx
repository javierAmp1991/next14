...
import DateItemAvailable from "./date-item-available/date-item-available";
...
export default function CalendarView({onSelectDate, onClose, datesAvailable, restrictions}:
                                         { onSelectDate: (date?: Date) => void, onClose?: Function, datesAvailable?: Date[], restrictions?: Date[] }) {
    const [dateSelected, setDateSelected] = useState<DateModal>({
        Days: getInitialDays(),
        Date: new Date(),
    ...
    return (
        <div className={style.grid}>
            <div className={style.contMont}>
               ...
               {/* <ImageComponent src={"/bgSummer.webp"} upStyle={style.bg}/>*/}
            ...
            <div className={style.main}>
                ...
                <div className={style.contDates}>
                   {/* {
                        (restrictions === undefined && datesAvailable === undefined) &&
                        dateSelected.Days.map((item: Date) =>
                            <DateItem Limit={limit} item={item}
                    ...
                    {
                        restrictions && dateSelected.Days.map((item: Date) =>
                            <DatItemRestricted Restrictions={restrictions} Limit={limit}
                                               onSelectDate={handleDateSelected} item={item}
                                               DateSelected={dateSelected.DateSelected}/>
                        )
                    }*/}
                    {
                        datesAvailable && dateSelected.Days.map((item: Date) =>
                            <DateItemAvailable DatesAvailable={datesAvailable} Limit={limit}
                                               onSelectDate={handleDateSelected} item={item}
                                               DateSelected={dateSelected.DateSelected}/>
                        )
                    }
    ...
    function handleDateSelected(item: Date) {
        const newDate: Date | undefined = item === dateSelected.DateSelected ? undefined : item;
        onSelectDate(newDate)
        setDateSelected({...dateSelected, DateSelected: newDate})
    }
